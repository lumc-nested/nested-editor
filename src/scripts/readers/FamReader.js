...
var readString = function(string) {
  ...
  nests = Immutable.Map(parser.getRelationships().filter(
    relationship =>
      members.has(relationship.MEMBER_1_ID.toString()) &&
      members.has(relationship.MEMBER_2_ID.toString())
  ).map(relationship => [
    Immutable.Set.of(
   ...
   * pregnancy has a TWIN_ID pointing to the second. All other childs in the
   ...
   * member we find its pregnancy instance and add the member to it if isn't
   * already (it might not be, but we could still find it via TWIN_ID). If
   * there's no pregnancy instance yet, we create it and add both the member
   * and TWIN_ID (if set) to it.
   *
